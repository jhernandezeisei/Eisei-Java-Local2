...
public class ProductoDaoImpl implements ProductoDao {
    ...
    public Producto crearClave(Producto pro) {
        try {
            Conexion c = new Conexion();
            con = c.getConnection();
            String sql = "{call SP_GenerarClave()}";
            ps = con.prepareStatement(sql);
            System.out.println(sql);
            ...
            while (rs.next()) {
                pro.setClave(rs.getString("codigo"));
...
@Override
    public List<Producto> catalogoList() {
        ...
        try {
            ...
            String sql = "{call SP_CatalogoFoto()}";
            ps = con.prepareCall(sql);
            rs = ps.executeQuery();

            while (rs.next()) {
                Producto catalogo = new Producto();
                catalogo.setNombreProducto(rs.getString("NombreProducto"));
                catalogo.setPrecio(rs.getFloat("Precio"));
                catalogo.setfotoProductoString(Base64.getEncoder().encodeToString(rs.getBytes("FotoProducto")));
                lista.add(catalogo);
            }
